...
void AssimpLoader::InitializeSkeleton(Skeleton* skeleton)
{
	...
	GetBoneNamesFromNode(boneNames);
...
// 
void AssimpLoader::GetBoneNamesFromNode(std::vector<std::string>& out_names)
{
...
void AssimpLoader::ExtractBoneTransform(aiNode* ainode, const Matrix44& accumulatedTransform, int parentBoneIndex, Skeleton* skeleton)
{
	...
	int suffixStart = (int) nodeName.find("_$AssimpFbx$_PreRotation"); // Name format is <BONENAME>_$AssimpFbx$_PreRotation
...
{
	if (firstFrameIndex >= (int) channel->mNumPositionKeys)
	{
	...
	if ((int) channel->mNumPositionKeys > firstFrameIndex)
	{
		bool found = false;
...
{
	if (tickOffset >= (int) channel->mNumRotationKeys)
	{
	...
	if ((int) channel->mNumRotationKeys > tickOffset)
	{
		bool found = false;
...
{
	if (tickOffset >= (int) channel->mNumScalingKeys)
	{
	...
	// Only bother interpolating if there's more than one key
	if ((int) channel->mNumScalingKeys > tickOffset)
	{
		bool found = false;
